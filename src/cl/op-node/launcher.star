...
def get_service_config(
    ...
    if op_node_major_version == 1 and op_node_minor_version >= 14:
        l1_genesis_artifact_name = "el_cl_genesis_data"  # default artifact name, generated by the ethereum package
        if l1_config_env_vars["L1_GENESIS_ARTIFACT_NAME"] != "":
            l1_genesis_artifact_name = "l1-genesis"  # default artifact name, when uploading a custom L1 genesis
        l1_genesis_original = plan.get_files_artifact(name=l1_genesis_artifact_name)
        result = plan.run_sh(
